...
from ex7_producer.settings import *
from ex7_producer.utils.functions import *
...
class Producer:
    """
    A class to produce messages from S3 CSV files to an SQS queue.
    """

    def __init__(self, bucket_name: str, queue_url: str, s3_path_prefix: str):
        """
        ...
        self.s3_client, self.sqs_client = self._get_aws_clients()
        self.parsed_schema = self._get_parsed_schema()


    @staticmethod
    def _get_aws_clients() -> Tuple[boto3.client, boto3.client]:
        """Initializes and returns Boto3 S3 and SQS clients."""
    ...
    def _send_chunk_to_sqs(self, chunk: List[Dict[str, Any]]):
        """Sends a chunk of rows as a single message to SQS."""
        try:
            # Send each record as a separate SQS message so consumers receive one record per message.
            ...
            for record in chunk:
                ...
                message_body = base64.b64encode(avro_binary_data).decode("utf-8")

                self.sqs_client.send_message(QueueUrl=self.queue_url, MessageBody=message_body)
            logger.info(f"Sent {len(chunk)} messages to SQS (one per record).")
        except ClientError as e:
    ...
    def process_file(self, object_key: str):
        """
        Processes a single CSV file and sends its data to SQS in chunks.
        """
        chunk = []
        for row_data in self._process_csv_rows(object_key):
            chunk.append(row_data)
            if len(chunk) >= CHUNK_SIZE:
                self._send_chunk_to_sqs(chunk)
                chunk = []
...
def main():
    ...
    try:
        if not all([S3_BUCKET_NAME, SQS_QUEUE_URL]):
            raise ValueError("S3_BUCKET_NAME and SQS_QUEUE_URL must be set.")

        producer = Producer(S3_BUCKET_NAME, SQS_QUEUE_URL, S3_PATH_PREFIX)
        producer.run()
